        ...
        <nav aria-label="Primary" className="flex items-center gap-1">
          {navItems.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="rounded-full px-3 py-2 text-sm font-medium text-[var(--primary)] hover:bg-[rgba(31,41,55,0.06)]"
            >
              {item.label}
            </a>
          ))}
          ...
          <a
            href="#contact"
            className="ml-2 inline-flex items-center rounded-full bg-[var(--secondary)] px-4 py-2 text-sm font-semibold text-white hover:opacity-90"
          >
            Let’s talk
          </a>
        </nav>
